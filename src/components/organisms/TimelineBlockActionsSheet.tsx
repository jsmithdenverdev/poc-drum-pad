...
export function TimelineBlockActionsSheet({
  ...
  // Use block values directly for display, local state for optimistic updates
  const [localPosition, setLocalPosition] = useState(block?.startMeasure ?? 0)
  const [localLength, setLocalLength] = useState(block?.lengthMeasures ?? 1)

  // Reset local state when sheet opens or block changes
  useEffect(() => {
    if (block) {
