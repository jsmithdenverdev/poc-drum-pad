...
interface TimelineWithScenesProps {
...
export function TimelineWithScenes({
  timeline,
  savedPatterns,
  playbackPosition,
  isPlaying,
  ...
  onScenePatternSet,
  className,
}: TimelineWithScenesProps) {
  const currentMeasure = Math.floor(playbackPosition)
  const playheadX = playbackPosition * MEASURE_WIDTH
  const scrollContainerRef = useRef<HTMLDivElement>(null)

  // View mode: 'timeline' or 'scenes'
  const [viewMode, setViewMode] = useState<'timeline' | 'scenes'>('scenes')

  // Focus mode: track which track to show (for timeline view)
  ...
  if (viewMode === 'scenes') {
    return (
      <div className={cn('flex flex-col h-full', className)}>
        ...
        <div className="flex-shrink-0 px-3 py-2 border-b border-border/30 bg-secondary/20 flex items-center justify-between">
        ...
        <SceneList
          ...
          onScenePatternSet={onScenePatternSet}
          className="flex-1"
        />
      </div>
    )
  ...
  // Render timeline view (original horizontal timeline)
  // Empty state
  if (!hasTracks) {
    return (
      <div className={cn('flex flex-col h-full', className)}>
